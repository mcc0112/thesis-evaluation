...
def load_csv(path: Path, variant: str) -> pd.DataFrame:
    ...
    if "instance" in df.columns:
        parsed = df["instance"].str.extract(r'n(\d+)_k(\d+)')
        df["config_n"] = pd.to_numeric(parsed[0], errors="coerce")
        df["config_k"] = pd.to_numeric(parsed[1], errors="coerce")
...
def compute_status_breakdown(df: pd.DataFrame) -> pd.DataFrame:
    if "status" not in df.columns:
        return pd.DataFrame()
    rows = []
    for (variant, n, k), grp in df.groupby(
            ["propagator_variant", "config_n", "config_k"]):
        total = len(grp)
        row = {
            "variant": variant, "config_n": n, "config_k": k,
            "config": config_label(n, k), "total": total,
        }
...
def compute_par2(df: pd.DataFrame, timeout_limit: int) -> pd.DataFrame:
    ...
    def par2_val(row):
        if row.get("status", "unknown") in OPTIMAL_STATUSES:
            try:
...
def aggregate(df: pd.DataFrame, col: str,
    ...
    if col not in filtered.columns:
        return pd.DataFrame(columns=[
            "propagator_variant", "config_n", "config_k", "median", "iqr", "n"])
    rows = []
    for (variant, n, k), grp in filtered.groupby(
            ["propagator_variant", "config_n", "config_k"]):
        vals = grp[col].dropna()
        n_solved = len(vals)
        if n_solved == 0:
        ...
        total_in_cell = len(df[
            (df["propagator_variant"] == variant) &
            (df["config_n"] == n) & (df["config_k"] == k)])
        if n_solved < MIN_SAMPLE_WARNING:
            print(f"  WARNING: [{variant}] {config_label(n, k)} – only "
...
def build_summary_table(df: pd.DataFrame, par2_df: pd.DataFrame,
    ...
    for (variant, n, k), grp in df.groupby(
        ...
        if not par2_df.empty:
            p = par2_df[(par2_df["variant"] == variant) &
                        (par2_df["config_n"] == n) &
                        (par2_df["config_k"] == k)]
            if len(p):
                par2_val = float(p.iloc[0]["median_par2"])
    ...
    summary = pd.DataFrame(rows).sort_values(["variant", "config_n", "config_k"])
    return summary
...
def plot_cross_hall_set_sizes(df: pd.DataFrame, out_path: Path) -> None:
    ...
    for (variant, col, label, color, ls) in series_spec:
        ...
        if not mask.any():
            continue
        sub = df[mask].groupby(["config_n", "config_k"])[col].median()
...
def plot_cross_depth_at_failure(df: pd.DataFrame, out_path: Path) -> None:
    ...
    for v in ["V0", "V1", "V2", "V3"]:
        ...
        for idx in df[mask].groupby(["config_n", "config_k"]).groups:
            all_configs_set.add(idx)
    ...
    for v in ["V0", "V1", "V2", "V3"]:
        ...
        sub  = df[mask & df["avg_depth_at_failure"].notna()].groupby(
            ["config_n", "config_k"])["avg_depth_at_failure"].median()
        ys = [float(sub.loc[c]) if c in sub.index else float("nan")
              for c in all_configs]
        if any(not np.isnan(y) for y in ys):
...
def run_analysis(df: pd.DataFrame, out_dir: Path, timeout_limit: int) -> None:
    ...
    for group_name, metric_dict in all_internal_groups:
        ...
        if existing:
            sub[["propagator_variant", "config_n", "config_k", "instance"]
                + existing].to_csv(
