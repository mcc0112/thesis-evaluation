...
# ---------------------------------------------------------------------------
# Variant-specific stat prefixes
# Maps solver_config (the part of the experiment dir name after the first "-")
# to a list of %%%mzn-stat key prefixes that should be bulk-collected.
#
# Variant 0: only circuit stats
# Variant 1: circuit + allDifferent (fewer allDifferent fields)
# Variant 2: circuit + allDifferent (more allDifferent fields)
# Variant 3: only circuit stats (different suffix/fields than V0)
#
# Replace the keys below with whatever strings actually appear as
# `solver_config` in your experiment directory names.
# ---------------------------------------------------------------------------
VARIANT_STAT_PREFIXES: Dict[str, List[str]] = {
    "pumpkin-v0": ["circuitNumber"],
    "pumpkin-v1": ["circuitNumber", "allDifferentNumber"],
    "pumpkin-v2": ["circuitNumber", "allDifferentNumber"],
    "pumpkin-v3": ["circuitNumber"],
}
...
def find_statistic_and_add_to_dict(log: str, statistic_name: str, statistics: Dict[str, str]):
    """Look up a single stat by exact name and store the last occurrence."""
...
def find_stats_by_prefix(log: str, prefix: str) -> Dict[str, str]:
    """
    Extract all %%%mzn-stat lines whose key starts with `prefix`,
    stripping the embedded constraint number and summing values
    across all instances of the same stat name.
    
    e.g. circuitNumber102PropagationsTotal and circuitNumber104PropagationsTotal
    both become circuitPropagationsTotal, and their values are summed.
    """
    stat_regex = re.compile(
        rf"%%%mzn-stat: ({re.escape(prefix)})(\d+)([^=\s]+)=([0-9]+\.?[0-9]*)"
    )
    results: Dict[str, float] = {}
    for match in stat_regex.finditer(log):
        prefix_part = match.group(1)   # e.g. "circuitNumber"
        # group(2) is the number — intentionally ignored
        suffix_part = match.group(3)   # e.g. "PropagationsTotal"
        value = float(match.group(4))
        key = prefix_part + suffix_part  # e.g. "circuitNumberPropagationsTotal"
        results[key] = results.get(key, 0.0) + value

    # Convert back to strings to match the original return type
    return {k: str(int(v) if v.is_integer() else v) for k, v in results.items()}
...
def run(
    ...
    for experiment in directories:
        ...
        print(f"DEBUG solver_config: '{solver_config}'")
        solver = CONFIG_TO_SOLVER[solver_config]
	

        # ------------------------------------------------------------------
        # Resolve which stat prefixes apply to this variant.
        # Falls back to an empty list if the config is not in the mapping,
        # so existing behaviour is fully preserved for unknown configs.
        # ------------------------------------------------------------------
        variant_prefixes = VARIANT_STAT_PREFIXES.get(solver_config, [])
        ...
        for item in tqdm(list(filter(lambda x: x.is_dir(), experiment.iterdir()))):
            ...
            # 1. Core fields (unchanged).
            # 2. Exact-name stats requested via --statistic CLI args.
            # 3. Prefix-based variant stats scraped from the full log.
            # ------------------------------------------------------------------
            stats = {
            ...
            # Exact-name stats (--statistic CLI flag), unchanged behaviour.
            for statistic in statistics:
                find_statistic_and_add_to_dict(log, statistic, stats)

            # Variant-specific prefix stats (e.g. circuitNumber*, allDifferentNumber*).
            # Each prefix is scraped independently; later prefixes can overwrite earlier
            # ones only if they share a key, which should never happen in practice.
            for prefix in variant_prefixes:
                stats.update(find_stats_by_prefix(log, prefix))
